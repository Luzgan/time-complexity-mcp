...
describe("Kotlin analyzer", () => {
  describe("kotlin-simple-loops.kt", () => {
    it("should detect O(n) for linearSearch", async () => {
      const result = await analyzeFile({ file_path: fixture("kotlin-simple-loops.kt") });
  ...
  describe("kotlin-nested-loops.kt", () => {
    it("should detect O(n^2) for bubble sort", async () => {
      const result = await analyzeFile({ file_path: fixture("kotlin-nested-loops.kt") });
  ...
  describe("kotlin-recursion.kt", () => {
    it("should detect O(n) for factorial (linear recursion)", async () => {
      const result = await analyzeFile({ file_path: fixture("kotlin-recursion.kt") });
  ...
  describe("kotlin-built-in-methods.kt", () => {
    it("should detect O(n log n) for sort", async () => {
      const result = await analyzeFile({ file_path: fixture("kotlin-built-in-methods.kt") });
      const fn = result.functions.find((f) => f.name === "sortList");
      expect(fn).toBeDefined();
      expect(fn!.complexity).toBe("O(n log n)");
    });

    it("should detect O(n^2) for indexOf inside loop", async () => {
      const result = await analyzeFile({ file_path: fixture("kotlin-built-in-methods.kt") });
