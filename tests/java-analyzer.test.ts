...
describe("Java analyzer", () => {
  describe("java-simple-loops.java", () => {
    it("should detect O(n) for linearSearch", async () => {
      const result = await analyzeFile({ file_path: fixture("java-simple-loops.java") });
  ...
  describe("java-nested-loops.java", () => {
    it("should detect O(n^2) for bubble sort", async () => {
      const result = await analyzeFile({ file_path: fixture("java-nested-loops.java") });
  ...
  describe("java-recursion.java", () => {
    it("should detect O(n) for factorial (linear recursion)", async () => {
      const result = await analyzeFile({ file_path: fixture("java-recursion.java") });
  ...
  describe("java-built-in-methods.java", () => {
    it("should detect O(n log n) for Collections.sort", async () => {
      const result = await analyzeFile({ file_path: fixture("java-built-in-methods.java") });
    ...
    it("should detect O(n) for forEach", async () => {
      const result = await analyzeFile({ file_path: fixture("java-built-in-methods.java") });
      const fn = result.functions.find((f) => f.name === "processAll");
      expect(fn).toBeDefined();
      expect(fn!.complexity).toBe("O(n)");
    });
  });
